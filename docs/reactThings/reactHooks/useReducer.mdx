...
function Counter() {
  ...
  return (
    <div>
      ...
      <br/>
      <button style={buttonStyle} onClick={() => dispatch({ type: 'INCREMENT', step })}>Increment</button>
      <button style={buttonStyle} onClick={() => dispatch({ type: 'DECREMENT', step })}>Decrement</button>
